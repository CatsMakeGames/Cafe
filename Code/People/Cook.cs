...
namespace Staff
{
	...
	public class Cook : Person
	{
		...
		{
			Salary = 100;
			textureSize = cafe.TextureManager.GetTexture("Cook").GetSize();
			size = new Vector2(128, 128);
			currentGoal = (Goal)saveData[12];
			goalOrderId = (int)saveData[13];
			currentApplianceId = saveData[14] == 0 ? -1 : (int)saveData[15] - 1;
			cafe.Connect(nameof(Cafe.OnNewOrderAdded),this,nameof(OnNewOrder));
			GenerateRIDBasedOnTexture(cafe.TextureManager.GetTexture("Cook"), ZOrderValues.Customer);
		}
		...
		public override void ResetOrCancelGoal(bool forceCancel = false)
		{
			...
			switch (currentGoal)
			{
				//find new appliance and move food there
				case Goal.CookFood:
